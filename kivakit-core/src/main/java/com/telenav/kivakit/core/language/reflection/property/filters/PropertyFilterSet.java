...
package com.telenav.kivakit.core.language.reflection.property.filters;
...
public class PropertyFilterSet implements PropertyFilter
{
    ...
    protected boolean isKivaKitIncluded(Field field)
    {
        ...
        {
            if (field.getAnnotation(KivaKitIncludeProperty.class) != null && included.contains(IncludeProperty.INCLUDED_FIELDS))
            {
                return true;
            }
            return field.getAnnotation(KivaKitPropertyConverter.class) != null && included.contains(IncludeProperty.CONVERTED_FIELDS_AND_METHODS);
        }
        return false;
    }
    ...
    protected boolean isKivaKitIncluded(Method method)
    {
        ...
        {
            if (method.getAnnotation(KivaKitPropertyConverter.class) != null && included.contains(IncludeProperty.CONVERTED_FIELDS_AND_METHODS))
            {
                return true;
            }

            if (method.getAnnotation(KivaKitIncludeProperty.class) != null && included.contains(IncludeProperty.INCLUDED_FIELDS_AND_METHODS))
            {
                return true;
            }

            if (included.contains(IncludeProperty.PUBLIC_METHODS) && Modifier.isPublic(method.getModifiers()))
            {
                return true;
            }

            return included.contains(IncludeProperty.NON_PUBLIC_METHODS);
        }
        return false;
    }
    ...
    protected boolean isSetter(Method method)
    {
        ...
        {
            return convention == NamingConvention.KIVAKIT || method.getName().startsWith("set");
        }
        return false;
    }
}
