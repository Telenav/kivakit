...
public class FrequencyTest extends CoreUnitTest
{
    ...
    {
        var now = now();
        var cycle = EVERY_10_SECONDS.startingAt(now);
    ...
    {
        ensureThrows(() -> parseFrequency(throwingListener(), "5"));
        ensureThrows(() -> parseFrequency(throwingListener(), "5 cows"));
        ensureThrows(() -> parseFrequency(throwingListener(), "every"));
        ensureThrows(() -> parseFrequency(throwingListener(), null));
        ensureThrows(() -> parseFrequency(throwingListener(), ""));

        ensureEqual(parseFrequency(throwingListener(), "every 5 seconds"), EVERY_5_SECONDS);
        ensureEqual(parseFrequency(throwingListener(), "every minute"), EVERY_MINUTE);
        ensureEqual(parseFrequency(throwingListener(), "every 30s"), EVERY_30_SECONDS);
        ensureEqual(parseFrequency(throwingListener(), "every 500ms"), EVERY_HALF_SECOND);
    }
