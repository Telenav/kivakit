...
public class HourOfWeekSpanTest extends CoreUnitTest
{
    ...
    public void testIncludes()
    {
        ...
        var mondayApril11at3amUtc = LocalTime.localTime(utc(),
                year(2022),
                APRIL,
                dayOfMonth(11),
                militaryHour(3),
                minute(0),
                second(0));
        var saturdayApril16at11pm = LocalTime.localTime(utc(),
                year(2022),
                APRIL,
                dayOfMonth(16),
                militaryHour(23),
                minute(0),
                second(0));
        for (var at = mondayApril11at3amUtc; at.isLessThanOrEqualTo(saturdayApril16at11pm); at = at.plus(days(1)))
        {
            ensure(utcSpan().includes(at));
        }

        // Center included: Monday at 3am to Saturday at 11pm in UTC should not include times after Saturday at 11pm
        var sundayApril10at1amUtc = LocalTime.localTime(utc(),
                year(2022),
                APRIL,
                dayOfMonth(10),
                militaryHour(1),
                minute(0),
                second(0));
        var sundayApril10at11pm = LocalTime.localTime(utc(),
                year(2022),
                APRIL,
                dayOfMonth(10),
                militaryHour(23),
                minute(0),
                second(0));
        for (var at = sundayApril10at1amUtc; at.isLessThanOrEqualTo(sundayApril10at11pm); at = at.plus(days(1)))
        {
            ensure(!utcSpan().includes(at));
        }
